...
async def gigshkdnnj(client, message):
    ...
    keybord = InlineKeyboardMarkup([[InlineKeyboardButton("الـــحــمــايـــه 𓃠", callback_data=f"jzhfjgh5")]])
...
async def h24mgdgbie(client: Client, CallbackQuery):
    ...
    button = [[InlineKeyboardButton(text="قفل الصور 𓃠", callback_data=f"stop_photo"), InlineKeyboardButton(text="فتح الصور 𓃠", callback_data=f"photoun")],[InlineKeyboardButton(text="قفل الفديو 𓃠", callback_data=f"stop_video"), InlineKeyboardButton(text="فتح الفديو 𓃠", callback_data=f"viddelet")],[InlineKeyboardButton(text="قفل التوجيه 𓃠", callback_data=f"stop_forward"), InlineKeyboardButton(text="فتح التوجيه 𓃠", callback_data=f"frwdelet")],[InlineKeyboardButton(text="قفل الروابط 𓃠", callback_data=f"stop_link"), InlineKeyboardButton(text="فتح الروابط 𓃠", callback_data=f"rwadelet")],[InlineKeyboardButton(text="قفل المنشن 𓃠", callback_data=f"stop_mention"), InlineKeyboardButton(text="فتح المنشن 𓃠", callback_data=f"mendelet")],[InlineKeyboardButton(text="قفل الملصقات 𓃠", callback_data=f"stop_sticker"), InlineKeyboardButton(text="فتح الملصقات 𓃠", callback_data=f"moldelet")],[InlineKeyboardButton(text="قفل الكل 𓃠", callback_data=f"stop_alkl"), InlineKeyboardButton(text="فتح الكل 𓃠", callback_data=f"opn_alkl")]]
...
async def welc_o_me(client, chat_member_updated):
    ...
    if user_id == target_user_id:
       try: 
        ...
        await client.send_photo(chat_member_updated.chat.id, photo=photosource, caption=f"**انضم مطور السورس الي هنا\nيرجي من الاعضاء احترامه𓃠♥**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"مطور السورس 𓃠♥", user_id=f"{user_id}")]]))    
       except Exception as e:
         await client.send_photo(chat_member_updated.chat.id, photo=photosource, caption=f"**انضم مطور السورس الي هنا\nيرجي من الاعضاء احترامه𓃠♥**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"مطور السورس 𓃠♥", user_id=f"{user_id}")]]))    
...
async def muttlock(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == OWNER_ID or message.from_user.username in caes:
      if message.chat.id in muttof:
        return await message.reply_text("التقييد معطل من قبل🔒")
