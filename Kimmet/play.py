...
async def join_call(bot_username, OWNER_ID, client, message, audio_file, group_id, vid, user_mention, photo, thum, namechat): 
    ...
    try:
        ...
        button = [[InlineKeyboardButton(text="ᴇɴᴅ", callback_data=f"stop"), InlineKeyboardButton(text="ꝛᴇsᴜᴍᴇ", callback_data=f"resume"), InlineKeyboardButton(text="ᴘᴇᴜsᴇ", callback_data=f"pause")], [InlineKeyboardButton(text="ᴄʜᴧɴɴᴇʟ", url=f"{soesh}"), InlineKeyboardButton(text="ɢꝛᴏᴜᴘ", url=f"{gr}")], [InlineKeyboardButton(text=f"{name}", url=f"https://t.me/{CASER}")], [InlineKeyboardButton(text="ᴧᴅᴅ ᴍᴇ", url=f"https://t.me/{bot_username}?startgroup=True")]]
        await client.send_photo(group_id, photo=photo, caption=f"**ᴧᴅᴅ sᴏɴɢ ᴛᴏ ᴘʟᴧʏ : {count}\n\nsᴏɴɢ ɴᴧᴍᴇ : {thum}\nʙʏ : {user_mention}\nɢꝛᴏᴜᴘ ʙʏ : [{namechat}]({loggerlink})**", reply_markup=InlineKeyboardMarkup(button), reply_to_message_id=message.id)
...
async def change_stream(bot_username, chat_id, client): 
    ...
    if chat_id in playlist and playlist[chat_id] and vidd and vidd[chat_id] and namecha and namecha[chat_id] and user_mentio and user_mentio[chat_id] and thu and thu[chat_id] and phot and phot[chat_id]:
        ...
        try:
            ...
            button = [[InlineKeyboardButton(text="ᴇɴᴅ", callback_data=f"stop"), InlineKeyboardButton(text="ꝛᴇsᴜᴍᴇ", callback_data=f"resume"), InlineKeyboardButton(text="ᴘᴇᴜsᴇ", callback_data=f"pause")], [InlineKeyboardButton(text="ᴄʜᴧɴɴᴇʟ", url=f"{soesh}"), InlineKeyboardButton(text="ɢꝛᴏᴜᴘ", url=f"{gr}")], [InlineKeyboardButton(text=f"{name}", url=f"https://t.me/{CASER}")], [InlineKeyboardButton(text="ᴧᴅᴅ ᴍᴇ", url=f"https://t.me/{bot_username}?startgroup=True")]]
            await apppp.send_photo(chat_id, photo=photo, caption=f"**ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n\nsᴏɴɢ ɴᴧᴍᴇ : {thum}\nʙʏ : {user_mention}\nɢꝛᴏᴜᴘ ʙʏ : [{namechat}]({loggerlink})**", reply_markup=InlineKeyboardMarkup(button))
            await apppp.send_message(logger, f"╭⎊ ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n│⎉ sᴏɴɢ ɴᴧᴍᴇ › {thum}\n│⎉  ʙʏ › {user_mention}\n│⎉  ɢꝛᴏᴜᴘ ʙʏ › [{namechat}]({loggerlink})\n╰⎊ [ѕᴏᴜꝛᴄᴇ]({soesh})\n", disable_web_page_preview=True)
        except Exception as e:
            pass
    else:
        try:
...
async def msonhfbg(client, message):
    ...
    if message.reply_to_message:
     ...
     if not c:
         return
     await client.send_photo(group_id, photo=photo, caption=f"**ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n\nsᴏɴɢ ɴᴧᴍᴇ : `{thum}`\nʙʏ : {user_mention}\nɢꝛᴏᴜᴘ ʙʏ : [{namechat}]({loggerlink})**", reply_markup=InlineKeyboardMarkup(button), reply_to_message_id=message.id)
     await client.send_message(logger, f"**⎊ ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n│⎉ sᴏɴɢ ɴᴧᴍᴇ › {thum}\n│⎉  ʙʏ › {user_mention}\n│⎉  ɢꝛᴏᴜᴘ ʙʏ › [{namechat}]({loggerlink})\n╰⎊ [ѕᴏᴜꝛᴄᴇ]({soesh})**", disable_web_page_preview=True)
    elif message.text:
     try:
    ...
    if not c:
         return
    await client.send_photo(group_id, photo=photo, caption=f"**ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n\nsᴏɴɢ ɴᴧᴍᴇ : `{thum}`\nʙʏ : {user_mention}\ɢꝛᴏᴜᴘ ʙʏ : [{namechat}]({loggerlink})**", reply_markup=InlineKeyboardMarkup(button), reply_to_message_id=message.id)
    await client.send_message(logger, f"**⎊ ᴘʟᴧʏɪɴɢ ɴᴏᴡ sᴛᴧꝛᴛᴇᴅ\n│⎉ sᴏɴɢ ɴᴧᴍᴇ › {thum}\n│⎉  ʙʏ › {user_mention}\n│⎉  ɢꝛᴏᴜᴘ ʙʏ › [{namechat}]({loggerlink})\n╰⎊ [ѕᴏᴜꝛᴄᴇ]({soesh})**", disable_web_page_preview=True)
    try:
         return
    except:
...
async def gers(client, message):
    ...
    global thu
    o = 1
    button = [[InlineKeyboardButton(text="ᴇɴᴅ", callback_data=f"stop"), InlineKeyboardButton(text="sᴋɪᴘ ⤶", callback_data=f"skip")]]
    group_id = message.chat.id
    if group_id in thu:
        count = len(thu[group_id])
        user_mentions = [str(user) for user in thu[group_id]]
        response = f"**⊰•━━━•[𓃠 ѕᴏᴜꝛᴄᴇ 𓃠]({soesh})•━━━•⊱\n\n╭⎊ ᴛʜᴇ sᴏɴɢs ᴏɴ ᴛʜᴇ ʟɪsᴛ:\n\n╰⎉ ɴᴜᴍʙᴇꝛ sᴏɴɢs › {count}\n\n**"
        if count == 0:
