        ...
        if message.new_chat_members[0].username == f"{CASER}":
            try:
                ...
                await client.send_message(message.chat.id, 
                    f"╮⎉ انضم [مطور البوت](https://t.me/{CASER}) للجروب الآن 𓃠\n╯⎊ يرجى من الأعضاء احترامه 🖤⚡️"
                )
            except Exception as e:
                await client.send_message(message.chat.id, f"╮⎉ انضم [مطور البوت](https://t.me/{CASER}) للجروب الآن 𓃠\n╯⎊ يرجى من الأعضاء احترامه 🖤⚡️")
    except Exception as e:
        print(e)
    ...
    if len(x) != 0:        
       ...
       if m.photo:
         async for photo in client.get_chat_photos(x[0],limit=1):
          await message.reply_photo(photo.file_id,caption=f"╭⎊ ᴋɪɴɢ › {m.first_name}\n│⎉ ᴜsᴇʀ › @{m.username}\n│⎉ ɪᴅ › {m.id}\n│⎉ ᴄʜᴀᴛ › {message.chat.title}\n╰⎊ ɪᴅ.ᴄʜᴀᴛ › {message.chat.id}",reply_markup=InlineKeyboardMarkup(
             [              
               [          
...
async def nummmm(client: client, message):
    ...
    if not chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] and not message.from_user.id == OWNER_ID and not message.from_user.username in caes:
     await message.reply("**♪ عذرا عزيزي هذا الامر للادمن الجروب فقط  𓃠 .**")
     return
    await message.reply_text("**♪ جاري بدأ المنشن ، لايقاف الامر اضغط /cancel  𓃠 .**")
    i = 0
    txt = ""
    zz = message.text
    if message.photo:
...
async def stop(client, message):
    ...
    if not chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] and not message.from_user.id == OWNER_ID and not message.from_user.username in caes:
     ...
     return
    if message.chat.id not in array:
      await message.reply("**♪ المنشن متوقف بي الفعل  𓃠 .**")
      return 
    if message.chat.id in array:
     array.remove(message.chat.id)
     await message.reply("**♪ تم ايقاف المنشن عزيزي  𓃠 .**")
     return                    
...
async def iddd(client, message):
    ...
    if message.from_user.photo:
        ...
        CASER = usr.username
        name = usr.first_name
        await message.reply_text(text=f"""╭⎊ᚐɴᴧᴍᴇ › {message.from_user.mention}\n╰⎉ᚐᴜsᴇꝛ › @{message.from_user.username}\n╭⎊ᚐɪᴅ › {message.from_user.id}\n╰⎉ᚐʙɪᴏ › {usr.bio}\n╭⎊ᚐᴄʜᴧᴛ › {message.chat.title}\n╰⎉ᚐɪᴅ.ɢꝛᴏᴜᴘ › {message.chat.id}""", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(name, url=f"https://t.me/{message.from_user.username}")]]))
    else:
        usr = await client.get_chat(message.from_user.id)
        user_id = usr.id
        name = usr.first_name
