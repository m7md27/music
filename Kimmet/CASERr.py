...
async def for_us65ers(client, message):
   ...
   button = [[InlineKeyboardButton(text="عـــربـــي 🇪🇬", callback_data=f"arbk"), InlineKeyboardButton(text="ᴇɴɢʟɪsʜ 🇺🇸", callback_data=f"english")],[InlineKeyboardButton(text=f"{namew}", url=f"https://t.me/{wenru}")], [InlineKeyboardButton(text="ᴀᴅᴅ ᴍᴇ 𓃠", url=f"https://t.me/{bot_username}?startgroup=True")]]
...
async def caesar_bot(client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("sᴏᴜꝛᴄᴇ 𓃠", url=f"{soesh}"),
                InlineKeyboardButton("ɢꝛᴏᴜᴘ 𓃠", url=f"{gr}"),
            ],
            [
                 InlineKeyboardButton("ᴅᴇᴠ 𓃠", url=f"https://t.me/{devus}")
            ],
            [ 
                 InlineKeyboardButton("ᴀᴅᴅ ᴍᴇ 𓃠", url=f"https://t.me/{bot_username}?startgroup=tru")
            ]
        ]
    )
