...
API_ID = int("17784375")
API_HASH = "f4979d23cf6e9c58fcf7886f123cdee6"
Bots = []
Musi = []
CASER = [] 
off =None
...
async def alivehi(client: Client, message):
    if message.from_user.username in CASER:
        return
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("جروب السورس𓃠", url=f"{group}"), InlineKeyboardButton("قناه السورس 𓃠", url=f"{source}")]])
...
async def caesar(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("مطور السورس 𓃠", url=f"https://t.me/{casery}"),
                InlineKeyboardButton("قناه السورس 𓃠", url=f"{source}"),
            ],
        ]
    )
...
async def admin_risghts(client: Client, CallbackQuery):
   ...
   if command == "CASER1":
    ...
    await user.send_message(logger, f"انا الان اعمل بنجاح 𓃠♥")
    await bot.send_message(logger, f"تم تنصيب البوت بنجاح 𓃠♥")
...
async def botstatfhgvhfus(client, message):
    ...
    try:
        ...
        return await message.reply_text("لا يوجد بوتات مصنوعه .𓃠")
    try:
        await message.reply_text(f"البوتات المصنوعة {d}\n📊 عدد المجموعات: {g}\n👤 عدد المستخدمين: {u}")
    except:
        await message.reply_text("لا يوجد بوتات مصنوعه .𓃠")
...
async def hchhG6M(client, message):
    ...
    await message.reply_text("**جاري تفعيل الاشتراك في جميع البوتات ..𓃠**")
    o = 0
    for x in get_Bots():
         o += 1
         bot_username = x[0]
...
async def hchhGu55M(client, message):
    ...
    if ask.text == "الغاء":
        return await ask.reply_text("تم الالغاء")
    channel = ask.text.replace("https://t.me/", "")
    await message.reply_text("**جاري تعطيل الاشتراك في جميع البوتات ..𓃠**")
    o = 0
    for x in get_Bots():
        o += 1
        bot_username = x[0]
...
async def hos(client, message):
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
        return
    await message.reply_text("**جاري ايقاف جميع البوتات ..𓃠**")
    o = 0
    for x in get_Bots():
        o += 1
        bot_username = x[0]
...
async def hos57304(client, message):
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
        return
    await message.reply_text("**جاري تحديث جميع البوتات ..𓃠**")
    o = 0
    for x in get_Bots():
        o += 1
        bot_username = x[0]
...
async def hossamGM(client, message):
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
        return
    await message.reply_text("**جاري حذف جميع البوتات ..𓃠**")
    o = 0
    for x in get_Bots():
        o += 1
        bot_username = x[0]
...
@app.on_message(filters.command("𓃠 قسم الاشتراك 𓃠", ""))
...
@app.on_message(filters.command("𓃠 قسم الاذاعه 𓃠", ""))
